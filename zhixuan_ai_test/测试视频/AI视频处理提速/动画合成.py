...
donghua_output = "xiayu.mov"
output_video = input_video + ".donghua.avi"
...
# 上下方
ffmpeg_cmd = f"""ffmpeg -stream_loop -1 -i {donghua_output} -i {input_video} -stream_loop -1 -i {donghua_output} -filter_complex "[0:v]format=argb,fps={video_fps},fade=in:st=0:d=1:alpha=1[ov];[1:v][ov]overlay=x=0:y=H*-0.8:eof_action=repeat:enable='lte(t,30)'[av];[2:v]fps={video_fps}[ov2];[av][ov2]overlay=x=0:y=H*0.8:eof_action=repeat:enable='lte(t,30)'" -map 1:a -c:a copy -c:v rawvideo -pix_fmt yuv420p -preset ultrafast -t {duration} -y {output_video}"""
# 左上角
