...
pipe = os.popen("ffprobe -v error -show_entries format=duration -of csv=p=0 %s" % "input.mp4")
...
# print(duration)
# 全屏
# ffmpeg_cmd = f"""ffmpeg -stream_loop -1 -i xiayu.mov -i input.mp4 -filter_complex "[0:v]format=argb,fade=in:st=0:d=1:alpha=1[ov];[1:v][ov]overlay=x=W-w:y=H-h:eof_action=repeat" -map 1:a -c:a copy -c:v rawvideo -pix_fmt yuv420p -preset ultrafast -t {duration} -y output.avi"""
# 顶部
...
# 右上角
ffmpeg_cmd = f"""ffmpeg -stream_loop -1 -i xiayu.mov -i input.mp4 -filter_complex "[0:v]format=argb,fade=in:st=0:d=1:alpha=1,scale=iw*1.5:ih*1.5,crop=1080*0.2:1920*0.2:1080*0.2:1920*0.2[ov];[1:v][ov]overlay=x=W-w:y=0:eof_action=repeat" -map 1:a -c:a copy -c:v rawvideo -pix_fmt yuv420p -preset ultrafast -t {duration} -y output.avi"""
# 左下角
# 右下角
# 底部
